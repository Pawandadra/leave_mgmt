...
app.post("/leave_mgmt/login", async (req, res) => {
  ...
  try {
    ...
    console.log(users[0]);
    console.log(users[0].department_name);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (leave_category === "short_leaves") {
        ...
        const [fromHours, fromMinutes, fromSeconds = 0] =
          secLeaveOption.fromTime.split(":").map(Number);
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      FROM faculty
      WHERE CONCAT(faculty_name, ' (', designation, ')') LIKE ? AND
      department_id = ?
      ORDER BY designation, faculty_name;
    `,
        [`%${search}%`, req.user.departmentId]
      );
